...
if __name__ == "__main__":
    ...
    if '/output/sd/' in args.input_csv:
        use_sd = True
    else:
        use_sd = False

    input_file_name = args.input_csv.split('/')[-1]
    if not use_sd:
        SAVE_DETECTED_AT = "detected_faces/evaluation_" + input_file_name.replace('.csv', '') + '/'
    else:
        SAVE_DETECTED_AT = "detected_faces/sd/evaluation_" + input_file_name.replace('.csv', '') + '/'
    ...
    if not use_sd:
        predidct_age_gender_race('./output/' + input_file_name.replace('.csv', '_output_raw.csv'), SAVE_DETECTED_AT)
    else:
        predidct_age_gender_race('./output/sd/' + input_file_name.replace('.csv', '_output_raw.csv'), SAVE_DETECTED_AT)
